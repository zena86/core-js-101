...
function isTriangle(a, b, c) {
  ...
  const minArr = [a, b, c].filter((el) => el !== max);
  return max < minArr[0] + minArr[1] || (a === b && b === c);
  // throw new Error('Not implemented');
}
...
function doRectanglesOverlap(rect1, rect2) {
  ...
  return (
    (condX1 || condX2 || condX3 || condX4)
    && (condY1 || condY2 || condY3 || condY4)
  );
  // throw new Error('Not implemented');
}
...
function getDigitalRoot(num) {
  ...
  const sum = numArr.reduce((acc, item) => acc + Number(item), 0);
  if (sum > 9) {
...
function getCommonDirectoryPath(pathes) {
  const minPathLen = Math.min(...pathes.map((x) => x.length));
  let commonStr = '';
  let result = '';
  for (let i = 0; i < minPathLen; i += 1) {
    const firstChar = pathes[0][i];
    const isEqual = pathes.map((x) => x[i]).every((x) => x === firstChar);
    if (!isEqual) {
      break;
    }
    commonStr += firstChar;
...
function getMatrixProduct(m1, m2) {
  return m1.map((row, i) => m2[0].map((_item, j) => row
    .reduce((acc, _el, n) => acc + m1[i][n] * m2[n][j], 0)));
  // throw new Error('Not implemented');
}
...
function evaluateTicTacToePosition(position) {
  ...
  const checkArr = (arr) => {
    const count = arr.reduce((acc, item) => acc + (item === 'X') - (item === '0'), 0);
    if (count === 3) { return 'X'; }
    if (count === -3) { return '0'; }
    return undefined;
  };
